...
int exec() {
  int cont = 1;
  int addr, val;
  void *jt[] = {&&LDI, &&LDO, &&LD, &&STO, &&ST, &&DEFAULT, &&ALU, &&SH, &&BR, &&BEQ, &&BGT, &&JI, &&JO, &&JDO, &&JDI, &&HALT};
  void *aluJt[] = {&&MOV, &&ADD, &&AND, &&INC, &&INCA, &&DEC, &&DECA, &&NOT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&GPC};
  if (insOpCode < 0 || insOpCode > 0xf) {
    goto DEFAULT;
  }
  goto *jt[insOpCode];
LDI: //case 0x0: // ld $i, d .............. 0d-- iiii iiii
  reg [insOp0] = insOpExt;
  goto DONE;
LDO: //case 0x1: // ld o(rs), rd .......... 1osd
  addr = (insOp0 << 2) + reg [insOp1];
  reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
  goto DONE;
LD: //case 0x2: // ld (rs, ri, 2), rd .... 2sid
  addr = reg [insOp0] + (reg [insOp1] << 2);
  reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
  goto DONE;
STO: //case 0x3: // st rs, o(rd) .......... 3sod
  ...
  mem [addr+3] = val       & 0xff;
  goto DONE;
ST: //case 0x4: // st rs, (rd, ri, 4) .... 4sdi
